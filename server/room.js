    ...
    return {
      ...
      draft: {
        ...
        managers: [...d.managers.values()].map((m) => ({
          ...
          roster: m.roster.map((u) => ({ id: u.id, name: u.name, disp: u.disp, first: u.first, club: u.club, code: u.code, position: u.position, price: u.price })),
          substitute: m.substitute && { name: m.substitute.name, disp: m.substitute.disp, first: m.substitute.first, club: m.substitute.club, code: m.substitute.code, position: m.substitute.position },
        })),
      },
    };
  }
