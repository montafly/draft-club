...
WELCOME = (
    ...
    "/lobby — драфты в наборе и подача заявки, /live — очки идущих матчей, "
    "/stop — отписаться, /status — статус."
)
BIND_HINT = (
...
BOT_COMMANDS = [
    ...
    {"command": "live", "description": "LIVE-очки идущих матчей"},
...
MAIN_KB = {"keyboard": [["/lobby", "/live"], ["/status", "/stop"]],
...
def handle_command(token: str, msg: dict) -> None:
    ...
    if low.startswith("/start"):
        ...
        tg.send_message(token, cid, "Команды: /start, /lobby, /live, /stop, /status.",
...
def live_match_ids() -> list[int]:
    """match_id матчей «в игре сейчас»: live/paused, либо pending, но уже начавшийся по времени
    (FanTeam в грозу/паузу сбрасывает статус в pending). Окно назад 8ч — чтобы не тянуть давно
    завершённые. Завершённые (confirmed/ended/cancelled) сюда не попадают — их закрывает results."""
    lo = quote((now_utc() - timedelta(hours=8)).isoformat(), safe="")
    hi = quote((now_utc() + timedelta(minutes=5)).isoformat(), safe="")
    rows = db.select("dc_matches",
                     f"select=match_id,start_time,status&start_time=gte.{lo}&start_time=lte.{hi}"
                     "&status=in.(pending,live,paused)&limit=200")
    out: list[int] = []
    for r in rows:
        if r.get("status") in ("live", "paused"):
            out.append(r["match_id"])
            continue
        t = r.get("start_time")                         # pending: в игре, если начался по времени
        try:
            if t and datetime.fromisoformat(t) <= now_utc():
                out.append(r["match_id"])
...
def send_live(token: str, chat_id: int) -> None:
    """Кнопка/команда /live: по запросу — снимок ТЕКУЩИХ очков по всем идущим матчам, где у
    участника есть игроки (свои и соперников по драфту). Без антидубля (по запросу шлём всегда),
    с лёгким кулдауном от двойного тапа. Данные обновляются collect раз в ~2 мин."""
    uid = bound_uid(chat_id)
    if not uid:
        tg.send_message(token, chat_id, "LIVE-очки доступны после привязки аккаунта." + BIND_HINT,
                        parse_mode="HTML", reply_markup=MAIN_KB)
        return
    now = time.time()
    if now - _live_cooldown.get(chat_id, 0) < LIVE_COOLDOWN_SEC:
        tg.send_message(token, chat_id, "Секунду — очки и так обновляются раз в ~2 мин.")
        return
    _live_cooldown[chat_id] = now
    tg.send_chat_action(token, chat_id, "typing")
    sent = 0
    for mid in live_match_ids():
        text = notify.build_user_message(mid, uid, kind="live")
        if not text:                                    # нет его игроков в этом матче
            continue
        res = tg.send_message(token, chat_id, text, parse_mode="HTML")
        if res.get("ok"):
            sent += 1
            print(f"[live/req] матч {mid} -> chat {chat_id} ({uid[:8]})")
        elif res.get("error_code") in (403, 400):
            upsert_subscriber({"id": chat_id}, active=False)
            return
    if not sent:
        tg.send_message(token, chat_id,
                        "Сейчас нет идущих матчей с твоими игроками. "
                        "Ближайшие матчи тура — в /status.", reply_markup=MAIN_KB)
